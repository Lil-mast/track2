...
address_street   TEXT    Borrowers only — street address
address_city     TEXT    Borrowers only — city
address_state    TEXT    Borrowers only — state/county
address_zip      TEXT    Borrowers only — postal code
risk_score       NUMERIC(5,2)  Borrowers only — AI-computed risk score 0–100
risk_level       ENUM    Borrowers only — 'low' | 'medium' | 'high' | 'critical' (cached from risk_score)
...
```
active → overdue → default → charged_off
active → paid_off
```
...
- `charged_off`: loan written off as unrecoverable loss
...
loan_number       TEXT        Human-readable ID (e.g. "LN-1042") — auto-generated by trigger
...
status            ENUM        active | overdue | default | paid_off | charged_off
risk_level        ENUM        low | medium | high | critical (derived from latest_risk_score)
collateral        TEXT        Optional — description of collateral securing the loan
purpose           TEXT        Why the loan was taken (e.g. "working_capital")
...
status      ENUM          scheduled | paid | missed | partial | failed
created_at  TIMESTAMPTZ
```
...
borrower_id          UUID          FK → users.id (denormalised for fast borrower-level queries)
...
status               ENUM          scheduled | paid | missed | partial | failed
payment_method       TEXT          Optional — e.g. "M-Pesa", "Bank Transfer"
confirmation_number  TEXT          Optional — unique payment reference (e.g. "QK7XYZ")
...
### Table 6: `lender_rules`

**What it stores:** Automation rules configured by a lender that trigger recovery actions when certain loan conditions are met.

**Why it exists:** Lenders need to encode their institutional policy as executable rules — e.g. "automatically send an email reminder when a loan is 7+ days overdue" or "flag for collections after 3 missed payments." Rules are evaluated by the recovery engine against every loan and drive automated or recommended actions.
...
trigger       ENUM          days_overdue | missed_payments | risk_score | balance_threshold
operator      ENUM          gte | lte | eq | gt | lt
threshold     NUMERIC(15,2) Condition value
action        ENUM          email_reminder | sms_reminder | phone_call | payment_plan |
                            hardship_review | legal_notice | collections_referral
priority      INTEGER       Lower = evaluated first
is_active     BOOLEAN       Whether rule is currently active
auto_execute  BOOLEAN       If true, fires without manual review
cooldown_days INTEGER       Min days between firings for the same loan
created_at    TIMESTAMPTZ
updated_at    TIMESTAMPTZ (auto-updated by trigger)
```
...
**What it stores:** A full action-level audit trail of every loan officer action — views, approvals, rule creation, exports, logins.

**Why it exists:** The `notifications` table is not an audit log — it drives the bell icon and email pipeline. A proper audit log records every user-initiated action with user identity, IP address, user agent, and change description for compliance purposes.
...
user_email    TEXT               Cached email
action        ENUM               login | view | create | update | delete |
                                 approve | reject | execute | export
entity_type   ENUM               borrower | loan | payment | recommendation |
                                 rule | user | settings
...
metadata      JSONB              Optional diff or extra context
created_at    TIMESTAMPTZ
```
...
recommended_action       TEXT             Discrete label for UI badge (e.g. "Restructuring Call")
recovery_action          ENUM             email_reminder | sms_reminder | phone_call | payment_plan |
                                          hardship_review | legal_notice | collections_referral
confidence_score         NUMERIC(4,3)     AI confidence 0.0–1.0
priority                 INTEGER          Lower = higher priority when multiple recommendations exist
expected_recovery_amount NUMERIC(15,2)    Projected amount recoverable
expected_recovery_rate   NUMERIC(5,2)     Projected recovery as percentage of outstanding
suggested_script         TEXT             Optional call/email script for the loan officer
status                   strategy_status  draft | pending | approved | rejected | executed |
                                          dispatched | expired
model_id                 TEXT             Which Nova model version drafted this
ai_model_version         TEXT             Model version string
reviewed_by              UUID             FK → users.id (who reviewed this recommendation)
reviewed_at              TIMESTAMPTZ      When it was reviewed
approved_at              TIMESTAMPTZ      Set when loan officer approves
executed_at              TIMESTAMPTZ      Set when the action is taken
expires_at               TIMESTAMPTZ      After this date the recommendation is stale
