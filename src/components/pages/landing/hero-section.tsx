...
export function HeroSection() {
  const [demoStatus, setDemoStatus] = useState<"idle" | "running" | "scheduled" | "completed">("idle");
  ...
  const triggerDemo = () => {
    ...
    // Simulate AI analysis steps in real-time
    setTimeout(() => {
      setRecoveryScore(92);
  ...
  return (
    <section className="relative max-w-7xl mx-auto px-6 pt-24 pb-20 flex flex-col items-center z-10">
      ...
      {/* Main Headline */}
      <h1 className="text-center font-extrabold text-5xl md:text-7xl text-white tracking-tight max-w-5xl leading-[1.1] mb-6">
        AI That Turns Debt Recovery Into <br />
      ...
      <p className="text-center text-slate-400 text-lg md:text-xl font-medium max-w-3xl leading-relaxed mb-10">
        RecoveryAI utilizes context-aware Generative AI to design hyper-personalized repayment strategies, maximizing recovery rates while safeguarding customer trust.
      </p>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-20 z-20">
        <button 
          onClick={triggerDemo}
          className="group relative inline-flex items-center gap-2 px-8 py-4 rounded-full bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-semibold shadow-lg shadow-blue-500/25 hover:shadow-cyan-500/30 hover:scale-[1.03] active:scale-[0.98] transition-all duration-300"
        >
        ...
        {demoStatus !== "idle" && (
          <button 
            onClick={resetDemo}
            className="px-6 py-4 rounded-full border border-slate-700 bg-slate-900/50 text-slate-300 hover:bg-slate-800/80 hover:text-white transition-all font-semibold"
          >
            Reset Simulation
          </button>
        )}
      ...
      <div className="w-full max-w-5xl rounded-2xl border border-white/[0.08] bg-[#0b0f19]/40 backdrop-blur-md p-6 shadow-2xl relative group/dashboard">
        {/* Colorful glowing aura behind the dashboard */}
        ...
        <div className="flex justify-between items-center border-b border-white/[0.08] pb-4 mb-6">
          ...
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${demoStatus === "running" ? "bg-amber-400 animate-ping" : "bg-emerald-500"}`} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="border border-white/[0.08] bg-slate-900/40 rounded-xl p-6 flex flex-col items-center justify-center relative overflow-hidden group/card hover:border-blue-500/30 transition-all duration-300">
            ...
            <div className="relative w-36 h-36 flex items-center justify-center mb-4">
              <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                <circle cx="50" cy="50" fill="transparent" r="40" stroke="rgba(255,255,255,0.04)" strokeWidth="6"></circle>
                <circle
                ...
                ></circle>
                <defs>
                  <linearGradient id="hero-gauge-grad" x1="0%" x2="100%" y1="0%" y2="100%">
                    <stop offset="0%" stopColor="#00d2ff"></stop>
            ...
            <div className="text-center relative z-10">
              <h4 className="text-sm font-semibold text-slate-300 mb-1">Portfolio Recovery Score</h4>
              <p className="text-xs text-slate-500">Auto-calculated risk-repayment ratio</p>
            </div>
          </div>
          ...
          <div className="md:col-span-2 border border-white/[0.08] bg-slate-900/40 rounded-xl p-6 flex flex-col justify-between group/card hover:border-cyan-500/30 transition-all duration-300 relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              <div>
                <h4 className="text-sm font-semibold text-slate-300">Recovery Trend (30 Days)</h4>
                <p className="text-xs text-slate-500">Comparing Standard vs RecoveryAI Performance</p>
              </div>
            ...
            <div className="relative h-32 w-full mt-2">
              <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 100 100">
                {/* Standard line */}
                <path d="M0,80 Q25,82 50,75 T100,70" fill="none" stroke="rgba(255,255,255,0.15)" strokeWidth="1.5" strokeDasharray="3 3"></path>
                
                {/* RecoveryAI dynamic curve */}
                <path
                  className="transition-all duration-1000"
                  d={demoStatus === "idle" ? "M0,80 Q25,60 50,45 T100,35" : "M0,80 Q25,45 50,25 T100,12"}
                  fill="none"
                  stroke="#3b82f6"
                  strokeWidth="2.5"
                ></path>
                ...
                <path
                  className="transition-all duration-1000"
                  d={demoStatus === "idle" ? "M0,80 Q25,60 50,45 T100,35 L100,100 L0,100 Z" : "M0,80 Q25,45 50,25 T100,12 L100,100 L0,100 Z"}
                  fill="url(#hero-trend-glow)"
                  opacity="0.1"
                ></path>

                <defs>
                  <linearGradient id="hero-trend-glow" x1="0%" x2="0%" y1="0%" y2="100%">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
          <div className="flex items-center gap-4 border border-white/[0.08] bg-slate-900/40 rounded-xl p-4 hover:border-slate-800 transition-colors">
            ...
            <div className="flex-1 min-w-0">
              <h5 className="text-xs font-bold text-white truncate">John Doe (Acme Capital)</h5>
              <p className="text-[10px] text-slate-400 truncate">Risk Status: <span className="text-emerald-400">Optimized</span> • Channel: SMS Nudge</p>
            </div>
            <div className="text-right shrink-0">
              <span className="text-xs font-bold text-slate-300">$1,250.00</span>
              <p className="text-[8px] text-slate-500 font-semibold uppercase tracking-wider">Balance</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 border border-white/[0.08] bg-slate-900/40 rounded-xl p-4 hover:border-slate-800 transition-colors relative overflow-hidden">
            ...
            <div className="w-8 h-8 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-400 shrink-0">
              <span className="material-symbols-outlined text-sm">
                {demoStatus === "completed" ? "check_circle" : "sync"}
              </span>
            </div>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-slate-500 truncate">
                {demoStatus === "idle" && "Click 'Trigger Live Demo' above to begin"}
                {demoStatus === "running" && "Reading context data with Amazon Nova"}
